...
$tsql = "select * from Log order by LogID";

$getResults= sqlsrv_query($conn, $tsql);
if (isset($_SESSION["UserID"])){
    $logResult = sqlsrv_query($conn, "insert into Log(UserID, ActionDescription) values (".$_SESSION["UserID"].",'Viewed log')");
}
echo ("Results:<br/>");
if ($getResults == FALSE) {
    ...
    while ($row = sqlsrv_fetch_array($resultSet, SQLSRV_FETCH_ASSOC)) {
        ...
        foreach($row as $col){
            ...
            if($col instanceof DateTime){
                $date=$col->setTimezone(new DateTimeZone('Europe/Athens'));
